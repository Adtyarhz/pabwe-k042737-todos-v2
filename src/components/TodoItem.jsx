...
function TodoItem({ todo, onDeleteTodo, onChangeCover }) {
...
${todo.title}?`,
          ...
          <div className="col-12 mt-2">
            {/* Tombol untuk mengganti cover */}
            <label
              htmlFor={`cover-${todo.id}`}
            ...
            <input
              type="file"
              id={`cover-${todo.id}`}
              onChange={(e) => onChangeCover(todo.id, e.target.files[0])} // Handle perubahan file cover
              style={{ display: "none" }}
            />
          </div>
        </div>
